...
app.get("/movies/", async (request, response) => {
  const getPlayersQuery = `SELECT * FROM movie;`;
  const moviesArray = await database.all(getPlayersQuery);
...
// API 2 TO get a Sepecific Movie
...
app.post("/movies/", async (request, response) => {
  ...
  const updatedQuery = await database.run(postmovieQuery);
...
app.get("/directors/", async (request, response) => {
  ...
  response.send(
    directorsArray.map((eachDirector) =>
      convertDirectorToResponseObject(eachDirector)
    )
  );
});
